...
def articles():
    ...
    # get loc
    loc = request.args.get("geo")

    # if not loc return error
    if not loc:
...
def search():
    ...
    # only show first 10 places
    if len(places) > 10:
        return jsonify([places[0], places[1], places[2],  places[3],  places[4],
