...
int main(int argc, char* argv[])
{
    ...
    if (argc != 4)
    {
        printf("Usage: resize multipler infile outfile\n");
        return 1;
    }
    ...
    char* infile = argv[2];
    char* outfile = argv[3];
    int multipler = atoi(argv[1]);
    
    if (multipler < 1 || multipler > 100)
    {
        printf("multipler must be in range 1..100!\n");
        return 1;
    }
    ...
    bi_r.biWidth  = bi.biWidth * multipler;
    bi_r.biHeight = bi.biHeight * multipler;
    ...
    {
        // Read each scanline multipler times
        ...
        {  
            ...
            for (int k = 0; k < bi.biWidth; k++)
            {
                ...
                // write RGB triple to outfile, multiplied by multipler
